...
export default function AIInnovation() {
  return (
    <section id="ai-innovation" className="py-[120px] bg-bg overflow-hidden">
      <div className="max-w-[1200px] mx-auto px-8">
        <div className="grid lg:grid-cols-2 gap-12 items-stretch">
          {/* Left: Dark Panel */}
          <div className="bg-dark rounded-[40px] p-12 lg:p-20 flex flex-col justify-center text-white">
          ...
          <div className="bg-white rounded-[40px] border border-border p-8 lg:p-16 flex flex-col">
            <div className="flex items-center justify-between mb-12">
              <h3 className="text-2xl font-bold text-dark">실시간 단가표</h3>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse" />
                <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest">Live Update</span>
              </div>
            </div>
              ...
              {[
                ...
                <div key={idx} className="flex items-center justify-between py-4 border-b border-border last:border-0">
                  ...
                  <div className="text-right">
                    <div className="text-lg font-bold text-dark">₩{item.price}</div>
                    <div className={cn(
                      "text-[10px] font-bold",
                      item.up === true ? "text-rose-500" : item.up === false ? "text-blue-500" : "text-muted"
                    )}>
                      {item.change}
                    </div>
                  </div>
            ...
            <button className="mt-12 w-full py-5 border border-dark text-dark font-bold rounded-2xl hover:bg-dark hover:text-white transition-all">
              전체 단가표 확인하기
            </button>
          </div>
        </div>
